...
After completing the setup, you only need one line `#include "bigint.h"` to start using our library~  
...
Once the environment is ready, you can obtain `bigint.h` in two ways:  
1. Click `bigint.h` in the repository, then click the **Raw** button in the top-right corner, and right-click to save as `bigint.h`.  
2. Copy and run:  
   `git clone https://gitee.com/cero0xA672/BIGINT.git`  
   `cd BIGINT`

Next, make sure `bigint.h` is in the same directory as your project file.  
In your project source file, write `#include "bigint.h"` — and you're ready to use our library!  
...
`fatal error: bigint.h: No such file or directory`: `bigint.h` is not in the same directory as your project file! Move it to the project directory!  
